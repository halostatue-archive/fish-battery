function battery -d 'Report on battery status for OS X and Linux'
    ...
    set -l filled ▮
    set -q _flag_filled; and set filled $_flag_filled

    set -l empty ▯
    set -q _flag_empty; and set empty $_flag_empty

    set -l low (set_color -o f00)
    set -l medium (set_color -o ff0)
    set -l high (set_color -o 0f0)

    set -q _flag_low; and set low $_flag_low
    set -q _flag_medium; and set medium $_flag_medium
    set -q _flag_high; and set high $_flag_high
    ...
    $__battery_is_plugged; and printf " "$plug
